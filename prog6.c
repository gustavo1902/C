...
void solve() {
	int r, c;
	read(r, c);
	vt<vt<int>> a(r, vt<int>(c));
	read(a);
	FOR(r)
		FOR(j, c)
			up[i][j]=a[i][j]?1+(i?up[i-1][j]:0):0;
	FOR(i, r-1, -1, -1)
		FOR(j, c)
			down[i][j]=a[i][j]?1+(i+1<r?down[i+1][j]:0):0;
	FOR(r)
		FOR(j, c)
			lft[i][j]=a[i][j]?1+(j?lft[i][j-1]:0):0;
	FOR(r)
		FOR(j, c-1, -1, -1)
			rght[i][j]=a[i][j]?1+(j+1<c?rght[i][j+1]:0):0;
	ll ans=0;
	FOR(r)
		FOR(j, c) {
